      ...
      <main id="main" className="page-container inner-page product-page">
        ...
        <header className="product-header">
          ...
          <dl>
            ...
            <div><dt>Versión</dt><dd>0.1</dd></div>
          </dl>
        </header>
        ...
        <section className="spec-section">
          <h2>Trazabilidad</h2>
          <div className="trace-grid"><div><span>Identificador</span><strong>P-{String(product.id).padStart(2, "0")}</strong></div><div><span>Fuente mínima</span><strong>{product.id === 4 ? "term_id + SRC-01 + página" : "entry_id + source_code + page"}</strong></div><div><span>Estado de datos</span><strong>{product.id === 1 ? "Transcrito" : product.id === 2 ? "Derivado de P-01" : product.id === 3 ? "Derivado de P-01 y P-02" : product.id === 4 ? "OCR estructurado de SRC-01" : "No materializado"}</strong></div><div><span>Validación lingüística</span><strong>Pendiente</strong></div></div>
        </section>

        {product.id === 1 ? <LexiconExplorer /> : product.id === 2 ? <CorpusExplorer /> : product.id === 3 ? <ParallelCorpusExplorer /> : product.id === 4 ? <TerminologyExplorer /> : (
